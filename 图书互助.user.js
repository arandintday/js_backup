	...
	if (isSearch&&notAdvsearch&&notBaoku) {
		...
		if ( isDuxiu||isCcelib) {
			...
			for (var i = 0; i < mybooks.length; i++) {
				...
				var scgAnchor="https://www.readersteam.com/ebook/?book_id="+ssid;
		...
		//深圳文献港，全国图书馆联盟搜索页
			...
			for (var i = 0; i < mybooks.length; i++) {
				...
				var myhref="https://www.readersteam.com/ebook/?book_id=";
				myhref+=ssid;
	...
	if (myurl.indexOf("bookDetail.jsp?") != -1) {
		...
		if (myurl.indexOf("/views/specific/") == -1) {
			...
			*/
			...
			var oldstr="bookDetail.jsp?dxNumber=";
			var newstr="book.do?go=showmorelib&type=1&dxid=";
			var newAnchor="https://www.readersteam.com/ebook/?book_id="+ssid;
