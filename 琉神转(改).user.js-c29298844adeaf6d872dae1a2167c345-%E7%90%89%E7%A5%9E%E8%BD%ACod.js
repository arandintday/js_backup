function processTxt(e) {
    var t, r = e;
    r && (r = r.replace(regObj.bdshare, linkArr.baidu), r = r.replace(regObj.bdshare1, linkArr.baidu),
            e = r, r = r.replace(/ *-?本站暂?不再?提供(文件)?下载-? */i, "").replace(/ *保护作者版权 */i, "").replace(/ *请支持正版 */i, "").replace(/\<\!--[^>]*>/g, ""),
            r = /^(magnet|bdcloud) /i.test(r) ? r : r.replace(/([a-z0-9]{5,}) +([a-z0-9]{5,})/g, "$1$2"),
            t = r,
            r = bdmd5(r),
            r = !/a\shref="magnet:|md5/i.test(r) ? r.replace(regObj.btih, linkArr.btih) : r,
            r = r.replace(regObj.yunpan, linkArr.yunpan),
            r = r.replace(regObj.howfile, linkArr.howf),
            r = r.replace(regObj.tcn, linkArr.tcn),
            r = r.replace(regObj.yyw, linkArr.yyw),
            r = r.replace(regObj.mega, linkArr.mega),
            r = r.replace(regObj.pixiv, linkArr.pixiv),
            r = r.replace(/(baidu\.com\/s\/[a-z\d]{7,23})#([\'\"])/gi, "$1$2"),
            r = r.replace(regObj.xunlei, linkArr.xunlei)
    );
    if (t != r) return r; else return e;
}

function bdmd5(str){
    var m = str.match(regObj.bdmd5);
    for(var k in m){
        if (m[k].indexOf('bdpan')===0){
            str = str.replace(m[k], linkArr.bdmd5.replace('$1',Base64.encode(m[k])).replace('$2',Base64.decode(m[k].replace(/\s*bdpan:\/\//g, ' ')).split('|')[0]));
        }else{
            str = str.replace(m[k], linkArr.bdmd5.replace('$1',Base64.encode(m[k])).replace('$2',m[k].substring(m[k].lastIndexOf("#")+1)));
        }
    }
    return str;
}
...
function changeUrl(e, t) {
    for (var r = e ? t[0] : ["a", "img"], a = 0, i = r.length; i > a; a++) for (var n = r[a], s = document.querySelectorAll(n), o = 0, l = s.length; l > o; o++) for (var p = 0, h = t[1].length; h > p; p++) {
...
var simpleRule = /(?:提取|访问)[码碼]?\s*[:：\s]?\s*([a-z\d]{4})/i,
    ...
    regObj = {
        bdmd5: /([0-9a-fA-F]{32}#[0-9a-fA-F]{32}#\d+#.*((?=(\n))?))|(bdpan:\/\/[0-9a-zA-Z+=]+)/g,
...
linkArr.btih = '$1<a href="magnet:?xt=urn:btih:$2" >磁链</a> → <a href="http://www.torrent.org.cn/home/convert/magnet2torrent.html?hash=$2" target="_blank" title="点击下载种子文件">【种子】</a> <a href="https://btso.pw/magnet/detail/hash/$2" target="_blank" title="支持Base32,可查看种子内容">【详情】</a>',
    linkArr.bdmd5 = "<a href='https://pan.baidu.com/disk/home#bdlink=$1' target='_blank'>秒传：$2</a>",
    linkArr.baidu = "$1<a href='http://pan.baidu.com/s/$2#$3' target='_blank'>度娘[$2]</a> $3 ",
    linkArr.yunpan = "<a href='$1' target='_blank'>云盘：$2</a>",
    ...
    linkArr.mega = "<a href='https://mega.co$1' target='_blank'>MEGA网盘</a>\n",
...
function elementPosition(o) {
    var t = 0, e = 0;
    if (o.offsetParent) for (t = o.offsetLeft, e = o.offsetTop; o.offsetParent;) o = o.offsetParent, t += o.offsetLeft, e += o.offsetTop;
    return {x: t, y: e}
}
...
function scrollToControl(o) {
    ...
    var e = t % 50, r = (t - e) / 50;
    scrollSmoothly(t, r, o), window.scrollBy(0, e)
}
...
function scrollSmoothly(o, t, a) {
    ...
    }, 10))) : (window.scrollTo(0, a), repeatCount = 0, void clearTimeout(cTimeout))
}

var repeatCount = 0, cTimeout;